...
def train(env, agents, network, logger, args):
    ...
    while step_count < args.total_steps:
        ...
        obs_batch = env.reset(args.render)
        ...
        for step in range(args.episode_steps):
            ...
            actions = network.action(obs_batch, eps, args.debug)

            # Act in the environment
            next_obs_batch, rewards, dones, info = env.step(actions, args.render)

            if args.debug:
                for i in range(batch_size):
                    print_info(agent_ids[i], obs_batch[i], actions[i], rewards[i], next_obs_batch[i], dones[i])

            # Process all experiences in one batch
            loss = network.step(obs_batch, actions, rewards, next_obs_batch, dones)
